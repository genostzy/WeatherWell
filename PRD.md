...
| Water-level reporting | **Built** | One tap per depth, with 3 seconds to undo. The confirmation says how many more neighbours must report before the barangay gets an advisory, counting only reports sent with a position as the engine does, and stays until the next tap |
...
| Calibration loop | **Built; no real flood yet** | Records each automatic advisory's outcome: confirmed or rejected by an official, expired without a verdict, or missed (an official raised the alert from nothing over reports the engine was counting that the floor bar would have acted on, so only the raised bar kept it quiet). At least two rejections, outnumbering confirmations since the bar last moved and within 90 days, raise that barangay's bar one step; a missed event lowers it one step, never below the floor. Every move goes in the action record as "Automatic — calibration", and officials see the outcomes and raised bars on the dashboard. The exit criterion needs a real flood; none has happened since it shipped |
...
| Scheduled jobs | **Built and active** | The engine and typhoon refresh run every 3 hours and the health check every 15 minutes, as GitHub Actions. GitHub's free runners start scheduled runs late (on 25 September the 15-minute check ran every 3 to 5 hours), so Supabase's `pg_cron` starts both through `workflow_dispatch`, with the token from Setup step 10. The first such run started on time at 02:30 UTC on 26 September and passed. GitHub's own schedule stays as a fallback, and the Monitor run fails, which emails the owner, once no on-time run has come for an hour (an expired token) |
