...
- [x] Each automatic advisory's outcome is recorded in `calibration_events`: confirmed, rejected, expired without a verdict, or missed. A miss is an official's alert raised from nothing over reports the engine was counting that the floor bar (3 reporters, trust 1.0) would have acted on, at a bar above the floor, so only the raised bar kept the engine quiet; the review of 26 September tightened it from "any located report" (`20260926062859`). Migrations `20260926010743_calibration_loop` and `20260926062859_missed_needs_floor_evidence`, tests `supabase/tests/calibration.sql` C1–C8.
