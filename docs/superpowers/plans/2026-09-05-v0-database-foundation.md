...
Task 6: the security advisor's "Function search_path mutable" finding on the five `tests.*` helpers (`as_user`, `as_anon`, `expect_denied`, `expect_allowed`, `expect_row_count`) was fixed rather than left, taking option (a) from the task-6 brief. Added `set search_path = ''` to all five. Every statement string these helpers `execute` (in `supabase/tests/rls.sql` and `supabase/tests/reference-tables.sql`) already used schema-qualified names (`public.*`, `private.*`), and the helpers' own bodies call only `pg_catalog` builtins (`set_config`, `current_setting`, `format`, `set local role`), which resolve regardless of `search_path` because `pg_catalog` is always implicitly searched. Re-ran the complete denial suite after the change (10/10 assertions passed, no `TSTFL`) to confirm the empty search path did not change how any caller-supplied statement resolved. The security advisor is now clean: zero findings, not just the four originally expected categories.
